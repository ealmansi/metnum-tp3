...
inline void CmdLine::_constructor()
{
	...
	if ( _helpAndVersion )
	{
		...
		SwitchArg* help = new SwitchArg("h","help",
						"Imprime información de uso y termina.",
						false, v);
		add( help );
		deleteOnExit(help);
		deleteOnExit(v);
	...
	SwitchArg* ignore  = new SwitchArg(Arg::flagStartString(),
			   ...
			   "Ignora el resto de los argumentos etiquetados despues de este flag.",
					   false, v);
	add( ignore );
	deleteOnExit(ignore);
	deleteOnExit(v);
...
inline void CmdLine::parse(int argc, char** argv)
{
  	...
  	for (int i = 0; static_cast<unsigned int>(i) < args.size(); i++)
	{
		...
		if ( !matched && !Arg::ignoreRest() )
			throw(CmdLineParseException("No se pudo identificar el argumento",
			                             args[i]));
    }

	if ( requiredCount < _numRequired )
		throw(CmdLineParseException("Falta al menos un argumento requerido!"));

	if ( requiredCount > _numRequired )
		throw(CmdLineParseException("Demasiados argumentos!"));
