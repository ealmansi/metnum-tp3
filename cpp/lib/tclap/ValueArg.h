...
bool ValueArg<T>::processArg(int *i, std::vector<std::string>& args)
{
    ...
    {
        if ( _alreadySet )
			throw( CmdLineParseException("El argumento ya se ingresó!", toString()) );

        if ( Arg::delimiter() != ' ' && value == "" )
			throw( ArgParseException( 
							"No se pudo encontrar el delimitador de este argumento!",
                             toString() ) );
        ...
        if ( value == "" )
        {
            ...
            if ( static_cast<unsigned int>(*i) < args.size() ) 
				...
				throw( ArgParseException("Falta un valor para este argumento!",
                                                    toString() ) );
        }
        else
			_extractValue( value );
